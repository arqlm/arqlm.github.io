...
def set_desurvey(collarfl, surveyfl, along_name, azimuth_name, inclination_name):
    ...
    for dhid in drillholes:
        ...
        drillholes[dhid].data = pd.DataFrame({'Along': surveyfl.data[along_name].loc[rows],
                                              'Azimuth': surveyfl.data[azimuth_name].loc[rows],
                                              'Inclination': surveyfl.data[inclination_name].loc[rows]})
...
def get_desurvey(datafl, drillholes, inplace=True, x='X', y='Y', z='Z', null=None):
    ...
    def desurvey(row):
        'Define a local desurvey function'
        # Get the corresponding drill hole, handle missing keys gracefully
        dhid = row[datafl.dh]
        if dhid not in drillholes:
            warnings.warn(f"Drillhole ID {dhid} not found in drillholes dictionary.")
            return pd.Series({x: null, y: null, z: null})
...
def get_comps(comps, datafl, vartypes='continuous', null=None, nprocess=None):
    ...
    grouped_comps = comps.groupby([datafl.dh], as_index=False)
    grouped_data = datafl.data.groupby([datafl.dh], as_index=False)
